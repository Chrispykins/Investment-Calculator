class DetailRow extends React.Component {
  ...
  render() {
    return React.createElement("span", {
      ...
      className: "cell age"
    }, this.props.age), React.createElement("span", {
      className: "cell investment"
      ...
      className: "cell total" + (this.props.last ? " last" : "")
    }, this.props.runningTotal));
    ;
  }
...
class DetailTable extends React.Component {
  ...
  render() {
    ...
    for (var i = 0; i < numYears; i++) {
      this.rows.push(React.createElement(DetailRow, {
        age: start + i,
    ...
    return React.createElement("div", {
      ...
      className: "cell investment"
    }, "Investment each year"), React.createElement("span", {
      className: "cell growth"
    }, "Growth each year"), React.createElement("span", {
      className: "cell total"
    }, "Running total")), this.rows);
  }
...
class App extends React.Component {
  ...
  render() {
    ...
    if (this.state.detailsShown) {
      ...
      var details = React.createElement(DetailTable, {
    ...
    return React.createElement("div", null, React.createElement("div", {
    ...
    }, "Investment Per Year: \xA0 $"), React.createElement("input", {
      className: "cell",
      id: "AnnualInvestment",
      type: "text",
      ref: this.investmentInput,
      ...
      className: "cell",
      htmlFor: "Growth"
    }, "Expected Growth:"), React.createElement("input", {
      className: "cell",
      id: "Growth",
      type: "text",
      ref: this.growthInput,
      ...
      className: "cell",
      htmlFor: "StartingAge"
    }, "Starting Age:"), React.createElement("input", {
      className: "cell",
      id: "StartingAge",
      type: "number",
      min: "0",
      max: highestAge,
      ref: this.startingInput,
      ...
      className: "cell",
      htmlFor: "EndingAge"
    }, "Ending Age:"), React.createElement("input", {
      className: "cell",
      id: "EndingAge",
      type: "number",
      min: "0",
      max: highestAge,
      ref: this.endingInput,
      ...
      className: "cell"
    }, "Funds at age ", this.state.endingAge, ":"), React.createElement("span", {
      className: "cell"
    }, formatMoney(this.state.endingFund)))), React.createElement("hr", null), React.createElement("h3", {
      id: "showDetails",
  ...
  resetInvestments(afterUpdate) {
    ...
    this.setState({
      investments: new Array(numYears).fill(investment)
    }, afterUpdate);
  }
  ...
  resetGrowths(afterUpdate) {
    ...
    this.setState({
      growths: new Array(numYears).fill(growth)
    }, afterUpdate);
  ...
  setGrowthSingleYear(value, year) {
    ...
    growths[year] = value;
    calculatorApp.current.setState({
      growths: growths
    }, function () {
      this.recalculate();
      this.checkGrowthArray();
    });
  }

  setStartingAge(event) {
    var startingAge = clampYear(event.target.value);
    this.startingInput.current.value = startingAge; //add/remove entries from the start of the list
  ...
  setEndingAge(event) {
    ...
    this.endingInput.current.value = endingAge; //add/remove entries from the end of the list
...
function formatMoney(number) {
  ...
  while (number >= 1000) {
    var remainder = number % 1000; //add leading zeros

    if (remainder < 10) remainder = "00" + remainder;else if (remainder < 100) remainder = "0" + remainder;
